(function(win, document) {
    ...
    Swap.prototype.rebuild = function(initial) {
        var self = this, cache = {};
        this.domListeners = [];

        $('['+this.config.defaultSelector+']').not('[data-swap-listens]').each(function() {
            var el = this,
            ...
            $.each(attributes, function(key, attr) {
                ...
                if (!initial)
                    self.checkOne(o, 'rebuild', cache, true);
            });

            $(el).data('swapListens', 'yes');
        });

        if (!initial) {
            this.lastCache = $.extend(this.lastCache, cache);
            this.cleanUp();
        }
    };
    ...
    Swap.prototype.checkOne = function(o, type, cache, force) {
        var self = this, cacheKey = o.conditional+(o.neg?'#':'##')+o.test;
